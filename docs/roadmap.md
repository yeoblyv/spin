...
**Have.** `.github/workflows/ci.yml` (gofmt/`go vet`/`go test -race` on `ubuntu-latest`/`macos-latest`/`windows-latest`, push/PR - see [§11](#11-cross-platform-compatibility)); `.github/workflows/release.yml` (cross-compiles darwin/linux/windows × amd64/arm64 from one Ubuntu runner on a `v*` tag push, embeds `version`/`commit` via `-ldflags` into `cmd/spin/main.go`'s own variables, verifies the linux/amd64 binary's `--version` output is byte-identical to the tag before publishing, computes a SHA-256 checksum per binary, publishes via `softprops/action-gh-release`); version is git-tag-only, `go.mod` carries no version field. The build and version-embedding steps were verified locally (cross-compiled for linux/amd64, and built+ran natively to confirm `--version` renders exactly `spin <version> (<commit>)`, matching what the workflow's own check compares against).

**Missing.** A public GitHub repository and a first tagged release to point the checksum-verified download flow at; the Spider-side installer that pulls a released `spin` binary on demand ([README](../README.md)'s stated model); the `.deb`/APT packaging step.
...
- **MUST** publish the `yeoblyv/spin` repository and push the existing local history. Not done — this is an owner action (creating and pushing to a public repository), not one this session takes unilaterally.
- **MUST** cut a first tagged release once [§1](#1-environment-bootstrap) and [§6](#6-interactive-console) are the only shipped commands, or once enough of [§2](#2-project-registry)–[§5](#5-php-runtime-management) lands to justify it — this is an owner call, not an engineering gate. The pipeline that makes a tag push actually produce a release now exists (`release.yml`); deciding to cut one is unchanged as an owner call.
- **MUST** build the Spider-side installer (referenced in `README.md`'s "Relationship to Spider" section) that resolves OS/arch, downloads the matching binary and checksum, verifies it, and installs it — this supersedes the deferred submodule/committed-binary options already ruled out. Not started — meaningless before a release exists to install.
- **MUST** confirm the chosen package name is actually free in each target channel before committing to it, not just check the GitHub repository name. The bare name `spin` is **already taken** in the official Debian/Ubuntu archives by an unrelated formal-verification tool ([R6]) present in `bullseye` through `sid`; a submission to those official archives would need a distinct source-package name (e.g. `spin-cli`, `spider-spin`). This does not block a self-hosted APT repository, where the source-package name is this project's own choice and the installed command can still be `spin` regardless. Already confirmed, as recorded here.
- **SHOULD** ship a `.deb` build (via `nfpm` or `goreleaser`, as an additional step alongside the existing cross-compiled binaries in `release.yml`) and a small self-hosted APT repository (e.g. published from a GitHub Pages branch), so `apt install` works immediately without waiting on any official-archive review. Not started.
